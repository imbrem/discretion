...
variable [DecidableEq α] [DecidableEq M] {f g : α →ᴮ (WithBot M)} {x y : α} {a : M}
...
def get (f : α →ᴮ (WithBot M)) (x : α) (hx : x ∈ f.support) : M
  := (f x).unbot (mem_support_iff.mp hx)
...
theorem get_eq_iff_eq_at (hx : x ∈ f.support) (hy : y ∈ f.support)
  ...
  simp only [get, WithBot.unbot_eq_iff]

/-- This is the same as `f.update x ↑a`, but with nicer defeqs for the support -/
def cons (x : α) (a : M) (f : α →ᴮ (WithBot M)) : α →ᴮ (WithBot M) where
...
@[simp, norm_cast]
theorem coe_cons : (f.cons x a : α → WithBot M) = Function.update f x a := rfl

@[simp]
theorem bot_cons : cons x a ⊥ = single x ↑a := by
  ext
  rw [single_eq_update]
  rfl
...
theorem cons_eq_update (x : α) (a : M) (f : α →ᴮ (WithBot M)) : cons x a f = f.update x a :=
  DFunLike.ext' rfl

@[simp]
theorem cons_eq_self (h : x ∈ f.support) : f.cons x (f.get x h) = f := by simp [cons_eq_update]

theorem cons_comm (f : α →ᴮ (WithBot M)) (h : x ≠ y) (a b : M) :
  cons x a (cons y b f) = cons y b (cons x a f) := by simp only [cons_eq_update, f.update_comm h]

@[simp]
theorem cons_idem (x : α) (a : M) (f : α →ᴮ (WithBot M)) : cons x a (cons x a f) = cons x a f :=
