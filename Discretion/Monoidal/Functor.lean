...
open Category Functor PremonoidalCategory MonoidalCategory'
...
def ofTensorHom : F.LaxMonoidal' where
  ...
  μ'_natural_left := fun f X' => by
    simp_rw [← MonoidalCategory'.tensorHom_id, ← F.map_id, μ'_natural]
  μ'_natural_right := fun X' f => by
    simp_rw [← MonoidalCategory'.id_tensorHom, ← F.map_id, μ'_natural]
  associativity' := fun X Y Z => by
    simp_rw [← MonoidalCategory'.tensorHom_id, ← MonoidalCategory'.id_tensorHom, associativity']
  left_unitality' := fun X => by
    simp_rw [← MonoidalCategory'.tensorHom_id, left_unitality']
  right_unitality' := fun X => by
    simp_rw [← MonoidalCategory'.id_tensorHom, right_unitality']
...
instance comp : (F ⋙ G).LaxMonoidal' where
  ...
  associativity' _ _ _ := by
    dsimp
    simp_rw [MonoidalCategory'.comp_whiskerRight, assoc, μ_natural_left_assoc,
...
instance comp : (F ⋙ G).OplaxMonoidal' where
  ...
  oplax_associativity' X Y Z := by
    dsimp
    rw [MonoidalCategory'.comp_whiskerRight, assoc, assoc, assoc, δ_natural_left_assoc, associativity,
...
lemma whiskerRight_ε_η (T : D) : ε F ▷ T ≫ η F ▷ T = 𝟙 _ := by
  rw [← MonoidalCategory'.comp_whiskerRight, ε_η, MonoidalCategory'.id_whiskerRight]

@[reassoc (attr := simp)]
lemma whiskerRight_η_ε (T : D) : η F ▷ T ≫ ε F ▷ T = 𝟙 _ := by
  rw [← MonoidalCategory'.comp_whiskerRight, η_ε, MonoidalCategory'.id_whiskerRight]

@[reassoc (attr := simp)]
lemma whiskerRight_μ_δ (X Y : C) (T : D) : μ F X Y ▷ T ≫ δ F X Y ▷ T = 𝟙 _ := by
  rw [← MonoidalCategory'.comp_whiskerRight, μ_δ, MonoidalCategory'.id_whiskerRight]

@[reassoc (attr := simp)]
lemma whiskerRight_δ_μ (X Y : C) (T : D) : δ F X Y ▷ T ≫ μ F X Y ▷ T = 𝟙 _ := by
  rw [← MonoidalCategory'.comp_whiskerRight, δ_μ, MonoidalCategory'.id_whiskerRight]
...
noncomputable def μNatIso :
    Functor.prod F F ⋙ MonoidalCategory'.tensor D ≅ MonoidalCategory'.tensor C ⋙ F :=
  NatIso.ofComponents (fun _ ↦ μIso F _ _)

/-- Monoidal functors commute with left tensoring up to isomorphism -/
@[simps!]
noncomputable def commTensorLeft (X : C) :
    F ⋙ MonoidalCategory'.tensorLeft (F.obj X) ≅ MonoidalCategory'.tensorLeft X ⋙ F :=
  NatIso.ofComponents (fun Y => μIso F X Y)

/-- Monoidal functors commute with right tensoring up to isomorphism -/
@[simps!]
noncomputable def commTensorRight (X : C) :
    F ⋙ MonoidalCategory'.tensorRight (F.obj X) ≅ MonoidalCategory'.tensorRight X ⋙ F :=
...
def toOplaxMonoidal : F.OplaxMonoidal' where
  ...
  oplax_left_unitality' _ := by
    rw [← cancel_epi (λ_ _).hom, Iso.hom_inv_id, h.left_unitality, assoc, assoc,
      Iso.map_hom_inv_id_assoc, Iso.hom_inv_id_assoc, MonoidalCategory'.hom_inv_whiskerRight]
  oplax_right_unitality' _ := by
    rw [← cancel_epi (ρ_ _).hom, Iso.hom_inv_id, h.right_unitality, assoc, assoc,
      Iso.map_hom_inv_id_assoc, Iso.hom_inv_id_assoc, MonoidalCategory'.whiskerLeft_hom_inv]
...
instance : (prod F G).LaxMonoidal' where
  ...
  μ'_natural_left _ _ := by
    ext
    all_goals
      simp only [prod_obj, MonoidalCategory'.prodMonoidal_tensorObj, prod_map,
        MonoidalCategory'.prodMonoidal_whiskerRight, prod_comp, μ_natural_left]
  μ'_natural_right _ _ := by
    ext
    all_goals
      simp only [prod_obj, MonoidalCategory'.prodMonoidal_tensorObj, prod_map,
      MonoidalCategory'.prodMonoidal_whiskerLeft, prod_comp,
        μ_natural_right]
  associativity' _ _ _ := by
    ext
    all_goals
      simp only [prod_obj, MonoidalCategory'.prodMonoidal_tensorObj,
      MonoidalCategory'.prodMonoidal_whiskerRight,
        MonoidalCategory'.prodMonoidal_associator, Iso.prod_hom, prod_map, prod_comp,
        LaxMonoidal'.associativity, MonoidalCategory'.prodMonoidal_whiskerLeft]
  left_unitality' _ := by
    ext
    all_goals
      simp only [MonoidalCategory'.prodMonoidal_tensorUnit, prod_obj,
        MonoidalCategory'.prodMonoidal_tensorObj,
        MonoidalCategory'.prodMonoidal_leftUnitor_hom_fst, LaxMonoidal'.left_unitality,
        MonoidalCategory'.prodMonoidal_whiskerRight,
        prod_map, MonoidalCategory'.prodMonoidal_leftUnitor_hom_snd, prod_comp]
  right_unitality' _ := by
    ext
    all_goals
      simp only [prod_obj, MonoidalCategory'.prodMonoidal_tensorUnit,
        MonoidalCategory'.prodMonoidal_tensorObj,
        MonoidalCategory'.prodMonoidal_rightUnitor_hom_fst, LaxMonoidal'.right_unitality,
        MonoidalCategory'.prodMonoidal_whiskerLeft,
        prod_map, MonoidalCategory'.prodMonoidal_rightUnitor_hom_snd, prod_comp]
...
instance Monoidal'.prod' [F.Monoidal'] [G.Monoidal'] :
  ...
  ε_η := by
    ...
    · simp only [prod_comp_fst, prod'_ε_fst', prod'_η_fst', ε_η,
        MonoidalCategory'.prodMonoidal_tensorUnit, prod_id]
  ...
  μ_δ _ _ := by
    ...
    · simp only [prod_comp_fst, prod'_μ_fst', prod'_δ_fst', μ_δ,
        prod'_obj, MonoidalCategory'.prodMonoidal_tensorObj, prod_id]
...
def rightAdjointLaxMonoidal' : G.LaxMonoidal' where
  ...
  associativity' X Y Z := (adj.homEquiv _ _).symm.injective (by
    ...
    erw [NatTrans.whiskerRight_app_tensor_app'_assoc adj.counit adj.counit,
      NatTrans.whiskerRight_app_tensor_app'_assoc adj.counit adj.counit]
    rw [MonoidalCategory'.tensorHom_def, assoc]
    dsimp
    rw [← MonoidalCategory'.comp_whiskerRight_assoc, left_triangle_components,
      MonoidalCategory'.id_whiskerRight, id_comp,
      MonoidalCategory'.whisker_exchange_assoc,
      MonoidalCategory'.whisker_exchange_assoc, ← MonoidalCategory'.tensorHom_def_assoc,
      MonoidalCategory'.associator_naturality, OplaxMonoidal'.associativity_assoc]
    rw [← δ_natural_right_assoc, ← δ_natural_right_assoc, ← δ_natural_right_assoc]
    nth_rw 4 [MonoidalCategory'.tensorHom_def]
    rw [← MonoidalCategory'.whisker_exchange, ← MonoidalCategory'.whiskerLeft_comp_assoc,
  ...
  left_unitality' X := (adj.homEquiv _ _).symm.injective (by
    ...
    rw [homEquiv_counit, homEquiv_counit, homEquiv_unit, homEquiv_unit,
      ...
      MonoidalCategory'.tensorHom_def, assoc, ← MonoidalCategory'.comp_whiskerRight_assoc,
      ← MonoidalCategory'.comp_whiskerRight_assoc, assoc, counit_naturality,
      left_triangle_components_assoc, MonoidalCategory'.id_whiskerLeft, assoc, assoc,
  ...
  right_unitality' X := (adj.homEquiv _ _).symm.injective (by
    ...
    rw [homEquiv_counit, homEquiv_unit, MonoidalCategory'.whiskerLeft_comp, homEquiv_unit,
      ...
      ← δ_natural_right_assoc, MonoidalCategory'.tensorHom_def, assoc,
      ← MonoidalCategory'.whisker_exchange_assoc,
...
lemma unit_app_tensor_comp_map_δ' (X Y : C) :
  ...
  simp only [← adj.unit_naturality_assoc, ← Functor.map_comp, ← δ_natural_assoc,
    ← tensor_comp, left_triangle_components, MonoidalCategory'.tensorHom_id,
    MonoidalCategory'.id_whiskerRight, comp_id]
...
instance isMonoidal_comp' {F' : D ⥤ E} {G' : E ⥤ D} (adj' : F' ⊣ G')
  ...
  leftAdjoint_μ X Y := by
    ...
    dsimp [homEquiv]
    rw [comp_counit_app, comp_counit_app, comp_counit_app, assoc,
        MonoidalCategory'.tensor_comp, δ_natural_assoc]
    dsimp
...
lemma counitIso_inv_app_tensor_comp_functor_map_δ_inverse' (X Y : C) :
  ...
  simp [← cancel_epi (e.unitIso.hom.app (X ⊗ Y)), Functor.map_comp,
    unitIso_hom_app_tensor_comp_inverse_map_δ_functor'_assoc]
...
instance isMonoidal_symm' [e.inverse.Monoidal'] [e.IsMonoidal'] :
  ...
  leftAdjoint_μ X Y := by
    simp only [toAdjunction, Adjunction.homEquiv_unit]
    dsimp [symm]
    rw [map_comp, counitIso_inv_app_tensor_comp_functor_map_δ_inverse'_assoc,
      ← Functor.map_comp, ← MonoidalCategory'.tensor_comp, Iso.hom_inv_id_app, Iso.hom_inv_id_app]
    dsimp
    rw [MonoidalCategory'.tensorHom_id, MonoidalCategory'.id_whiskerRight, map_id, comp_id]
