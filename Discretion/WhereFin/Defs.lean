...
/-- `WhereFin α M p`, is the type of functions `f : α → M` such that `p (f x)` for only
  finitely many `x`. -/
structure WhereFin (α : Type*) (M : Type*) (p : Set M) where
  /-- The support on which this function satisfies `p` -/
  support : Finset α
  /-- The underlying function -/
  toFun : α → M
  /-- The witness that the support of a `WhereFin` is indeed the exact locus where its
  underlying function satisfies `p`. -/
  mem_support_toFun : ∀ a, a ∈ support ↔ p (toFun a)
...
instance instFunLike : FunLike (WhereFin α M p) α M :=
  ⟨toFun, by
...
directly. -/
instance instCoeFun : CoeFun (WhereFin α M p) fun _ => α → M :=
  inferInstance

@[ext]
theorem ext {f g : WhereFin α M p} (h : ∀ a, f a = g a) : f = g :=
  DFunLike.ext _ _ h

@[simp, norm_cast]
theorem coe_mk (f : α → M) (s : Finset α) (h : ∀ a, a ∈ s ↔ p (f a))
  : ⇑(⟨s, f, h⟩ : WhereFin α M p) = f :=
  rfl
...
/-- `always z` is the function with value `z` everywhere, where `z` satisfies `¬p` -/
def always (z : M) (hz : ¬p z) : WhereFin α M p where
  support := ∅
  toFun := λ_ => z
  mem_support_toFun := by simp [hz]

/-- `empty z` is the function with value `z` everywhere -/
def empty (z : M) : WhereFin α M (· ≠ z) := always z (by simp)

instance instInhabited : Inhabited (WhereFin α M (· ≠ z)) := ⟨empty z⟩

theorem default_apply {a : α} : (default : WhereFin α M (· ≠ z)) a = z :=
  rfl

@[simp]
theorem mem_support_iff {f : WhereFin α M p} : ∀ {a : α}, a ∈ f.support ↔ p (f a) :=
  @(f.mem_support_toFun)

@[simp, norm_cast]
theorem fun_support_eq (f : WhereFin α M p) : f ⁻¹' p = f.support :=
  Set.ext fun _x => mem_support_iff.symm

theorem not_mem_support_iff {f : WhereFin α M p} {a} : a ∉ f.support ↔ ¬(p (f a)) :=
  by simp

theorem not_mem_support_iff_eq {f : WhereFin α M (· ≠ z)} {a} : a ∉ f.support ↔ f a = z :=
  by simp

theorem ext_iff' {f g : WhereFin α M (· ≠ z)}
  : f = g ↔ f.support = g.support ∧ ∀ x ∈ f.support, f x = g x :=
  ⟨fun h => h ▸ ⟨rfl, fun _ _ => rfl⟩, fun ⟨h₁, h₂⟩ =>
    ext fun a => by
      classical
      exact if h : a ∈ f.support then h₂ a h else by
        have hf : f a = z := not_mem_support_iff_eq.mp h
        have hg : g a = z := by rwa [h₁, not_mem_support_iff_eq] at h
        rw [hf, hg]⟩

@[simp]
theorem support_eq_empty {f : WhereFin α M (· ≠ z)} : f.support = ∅ ↔ f = default :=
  by simp [Finset.ext_iff, DFunLike.ext_iff, default_apply]

theorem support_nonempty_iff {f : WhereFin α M (· ≠ z)} : f.support.Nonempty ↔ f ≠ default := by
  simp only [support_eq_empty, Finset.nonempty_iff_ne_empty, Ne]

theorem card_support_eq_zero {f : WhereFin α M (· ≠ z)} : card f.support = 0 ↔ f = default := by
  simp

instance instDecidableEq [DecidableEq α] [DecidableEq M] : DecidableEq (WhereFin α M (· ≠ z))
  := fun f g => decidable_of_iff (f.support = g.support ∧ ∀ a ∈ f.support, f a = g a) ext_iff'.symm

theorem support_subset_iff {s : Set α} {f : WhereFin α M p} :
    ↑f.support ⊆ s ↔ ∀ a ∉ s, ¬p (f a) := by
  simp only [Set.subset_def, mem_coe, mem_support_iff]; exact forall_congr' fun a => by
    rw [not_imp_comm, not_not]

theorem support_subset_iff_eq {s : Set α} {f : WhereFin α M (· ≠ z)} :
    ↑f.support ⊆ s ↔ ∀ a ∉ s, f a = z := by
  simp only [Set.subset_def, mem_coe, mem_support_iff]; exact forall_congr' fun a => not_imp_comm

-- TODO: this can actually be made computable, where `p` is...
/-- Given `Finite α`, `equivFunOnFinite` is the `Equiv` between `WhereFin α β p` and `α → β`.
  (All functions on a finite type are finitely supported.) -/
@[simps]
noncomputable def equivFunOnFinite [Finite α] : (WhereFin α M p) ≃ (α → M) where
  toFun := (⇑)
  invFun f := mk (f ⁻¹' p).toFinite.toFinset f fun _a => Set.Finite.mem_toFinset _
  left_inv _f := ext fun _x => rfl
  right_inv _f := rfl

@[simp]
theorem equivFunOnFinite_symm_coe {α} [Finite α] (f : WhereFin α M p)
  : equivFunOnFinite.symm f = f :=
  equivFunOnFinite.symm_apply_apply f

/--
If `α` has a unique term, `WhereFin α β p` is equivalent to `β`.
-/
@[simps!]
noncomputable def _root_.Equiv.whereFinUnique {ι : Type*} [Unique ι] : (WhereFin ι M p) ≃ M :=
  WhereFin.equivFunOnFinite.trans (Equiv.funUnique ι M)

@[ext]
theorem unique_ext [Unique α] {f g : WhereFin α M p} (h : f default = g default) : f = g :=
  ext fun a => by rwa [Unique.eq_default a]

theorem unique_ext_iff [Unique α] {f g : WhereFin α M p} : f = g ↔ f default = g default :=
...
-- variable {z : M} [DecidableEq α] [DecidableEq β] [DecidableEq M] {a a' : α} {b : M}

-- /-- `single a b` is the finitely supported function with value `b` at `a` and `z` otherwise. -/
-- -- NOTE: we flipped the inequality here to get defeq w/ Function.update
-- def single (z : M) (a : α) (b : M) : WhereFin α M (· ≠ z) where
...
-- @[simp]
-- theorem single_eq_same : (single a b : α →ᵀ M) a = b := by simp [single_apply]

-- -- TODO: flip inequality
-- @[simp]
-- theorem single_eq_of_ne (h : a ≠ a') : (single a b : α →ᵀ M) a' = ⊤
...
-- @[simp]
-- theorem single_top (a : α) : (single a ⊤ : α →ᵀ M) = ⊤ :=
--   DFunLike.coe_injective <| by
--     classical simpa only [single_eq_update, coe_top] using Function.update_eq_self a (⊤ : α → M)
...
-- theorem support_single_ne_top (a : α) (hb : b ≠ ⊤) : (single a b).support = {a} :=
--   if_neg hb
...
-- /-- `FinsuppTop.single a b` is injective in `b`. For the statement that it is injective in `a`, see
-- `FinsuppTop.single_left_injective` -/
-- theorem single_injective (a : α) : Function.Injective (single a : M → α →ᵀ M) := fun b₁ b₂ eq => by
--   have : (single a b₁ : α →ᵀ M) a = (single a b₂ : α →ᵀ M) a := by rw [eq]
--   rwa [single_eq_same, single_eq_same] at this

-- theorem single_apply_eq_top {a x : α} {b : M} : single a b x = ⊤ ↔ x = a → b = ⊤ := by
--   simp [single_apply]

-- theorem single_apply_ne_top {a x : α} {b : M} : single a b x ≠ ⊤ ↔ x = a ∧ b ≠ ⊤ := by
--   simp [single_apply_eq_top]

-- theorem mem_support_single (a a' : α) (b : M) : a ∈ (single a' b).support ↔ a = a' ∧ b ≠ ⊤ := by
--   simp [single_apply_eq_top, not_or]

-- theorem eq_single_iff {f : α →ᵀ M} {a b} : f = single a b ↔ f.support ⊆ {a} ∧ f a = b := by
...
--   simpa only [support_single_ne_top _ h] using singleton_ne_empty _

-- theorem support_single_disjoint {b' : M} (hb : b ≠ ⊤) (hb' : b' ≠ ⊤) {i j : α} :
--     Disjoint (single i b).support (single j b').support ↔ i ≠ j := by
--   rw [support_single_ne_top _ hb, support_single_ne_top _ hb', disjoint_singleton]

-- @[simp]
-- theorem single_eq_top : single a b = ⊤ ↔ b = ⊤ := by simp [DFunLike.ext_iff, single_apply]

-- theorem single_swap (a₁ a₂ : α) (b : M) : single a₁ b a₂ = single a₂ b a₁ := by
--   simp only [single_apply, eq_comm]

-- theorem unique_single [Unique α] (x : α →ᵀ M) : x = single default (x default) :=
...
--     ⟨_, h.1, h.2⟩,
--     fun ⟨_b, hb, hf⟩ => hf.symm ▸ support_single_ne_top _ hb⟩

-- theorem card_support_eq_one {f : α →ᵀ M} : card f.support = 1 ↔ ∃ a, f a ≠ ⊤ ∧ f = single a (f a) :=
--   by simp only [card_eq_one, support_eq_singleton]

-- theorem card_support_eq_one' {f : α →ᵀ M} :
--     card f.support = 1 ↔ ∃ a, ∃ b ≠ ⊤, f = single a b := by
--   simp only [card_eq_one, support_eq_singleton']

-- theorem support_subset_singleton {f : α →ᵀ M} {a : α} : f.support ⊆ {a} ↔ f = single a (f a) :=
--   ⟨fun h => eq_single_iff.mpr ⟨h, rfl⟩, fun h => (eq_single_iff.mp h).left⟩

-- theorem support_subset_singleton' {f : α →ᵀ M} {a : α} : f.support ⊆ {a} ↔ ∃ b, f = single a b :=
--   ⟨fun h => ⟨f a, support_subset_singleton.mp h⟩, fun ⟨b, hb⟩ => by
--     rw [hb, support_subset_singleton, single_eq_same]⟩

-- theorem card_support_le_one [Nonempty α] {f : α →ᵀ M} :
...
-- instance instNontrivial [Top M] [Nonempty α] [Nontrivial M] : Nontrivial (α →ᵀ M) := by
--   classical
--   inhabit α
--   rcases exists_ne (⊤ : M) with ⟨x, hx⟩
--   exact nontrivial_of_ne (single default x) ⊤ (mt single_eq_top.1 hx)
...
-- variable [Top M] [DecidableEq α] [DecidableEq β] [DecidableEq M]
--   (f : α →ᵀ M) (a : α) (b : M) (i : α)
...
-- def update (f : α →ᵀ M) (a : α) (b : M) : α →ᵀ M where
...
-- @[simp]
-- theorem top_update : update ⊤ a b = single a b := by
--   ext
--   rw [single_eq_update]
--   rfl
...
-- @[simp]
-- theorem support_update_top : support (f.update a ⊤) = f.support.erase a := by
--   simp [update]

-- variable {b}

-- theorem support_update_ne_top (h : b ≠ ⊤) :
...
-- -/
-- def erase (a : α) (f : α →ᵀ M) : α →ᵀ M where
...
-- @[simp]
-- theorem support_erase [DecidableEq α] {a : α} {f : α →ᵀ M} :
--   (f.erase a).support = f.support.erase a := rfl

-- @[simp]
-- theorem erase_same {a : α} {f : α →ᵀ M} : (f.erase a) a = ⊤ := by
--   simp only [erase, coe_mk, ite_true]

-- @[simp]
-- theorem erase_ne {a a' : α} {f : α →ᵀ M} (h : a' ≠ a) : (f.erase a) a' = f a' := by
--   simp only [erase, coe_mk, h, ite_false]

-- theorem erase_apply {a a' : α} {f : α →ᵀ M} :
...
-- @[simp]
-- theorem erase_of_not_mem_support {f : α →ᵀ M} {a} (haf : a ∉ f.support) : erase a f = f := by
...
-- theorem erase_top (a : α) : erase a (⊤ : α →ᵀ M) = ⊤ := by
--   rw [← support_eq_empty, support_erase, support_top, erase_empty]

-- theorem erase_eq_update_top (f : α →ᵀ M) (a : α) : f.erase a = update f a ⊤ := by
--   ext; simp [erase_apply, update_apply]

-- -- The name matches `Finset.erase_insert_of_ne`
-- theorem erase_update_of_ne (f : α →ᵀ M) {a a' : α} (ha : a ≠ a') (b : M) :
--     erase a (update f a' b) = update (erase a f) a' b := by
--   rw [erase_eq_update_top, erase_eq_update_top, update_comm _ ha]

-- -- not `simp` as `erase_of_not_mem_support` can prove this
-- theorem erase_idem (f : α →ᵀ M) (a : α) :
--     erase a (erase a f) = erase a f := by
--   rw [erase_eq_update_top, erase_eq_update_top, update_idem]

-- @[simp] theorem update_erase_eq_update (f : α →ᵀ M) (a : α) (b : M) :
--     update (erase a f) a b = update f a b := by
--   rw [erase_eq_update_top, update_idem]

-- @[simp] theorem erase_update_eq_erase (f : α →ᵀ M) (a : α) (b : M) :
...
--   [DecidableEq M] [DecidableEq N] [DecidableEq P] [Top M] [Top N] [Top P]

-- /-- The composition of `f : M → N` and `g : α →ᵀ M` is `mapRange f hf g : α →₀ N`,
-- which is well-defined when `f ⊤ = ⊤`.
-- -/
-- def mapRange (f : M → N) (hf : f ⊤ = ⊤) (g : α →ᵀ M) : α →ᵀ N :=
...
-- variable [DecidableEq α] [DecidableEq β] [DecidableEq M] [DecidableEq N] [Top M] [Top N]

-- /-- Given `f : α ↪ β` and `v : α →ᵀ M`, `Finsupp.embDomain f v : β →ᵀ M`
-- is the finitely supported function whose value at `f a : β` is `v a`.
-- For a `b : β` outside the range of `f`, it is `⊤`. -/
-- def embDomain (f : α ↪ β) (v : α →ᵀ M) : β →ᵀ M where
...
-- @[simp]
-- theorem support_embDomain (f : α ↪ β) (v : α →ᵀ M) : (embDomain f v).support = v.support.map f :=
--   rfl

-- @[simp]
-- theorem embDomain_top (f : α ↪ β) : (embDomain f ⊤ : β →ᵀ M) = ⊤ :=
--   rfl

-- @[simp]
-- theorem embDomain_apply (f : α ↪ β) (v : α →ᵀ M) (a : α) : embDomain f v (f a) = v a := by
--   classical
--     change dite _ _ _ = _
...
-- theorem embDomain_notin_range (f : α ↪ β) (v : α →ᵀ M) (a : β) (h : a ∉ Set.range f) :
--     embDomain f v a = ⊤ := by
--   classical
...
-- theorem embDomain_injective (f : α ↪ β) : Function.Injective (embDomain f : (α →ᵀ M) → β →ᵀ M) :=
--   fun l₁ l₂ h => ext fun a => by simpa only [embDomain_apply] using DFunLike.ext_iff.1 h (f a)

-- @[simp]
-- theorem embDomain_inj {f : α ↪ β} {l₁ l₂ : α →ᵀ M} : embDomain f l₁ = embDomain f l₂ ↔ l₁ = l₂ :=
--   (embDomain_injective f).eq_iff

-- @[simp]
-- theorem embDomain_eq_zero {f : α ↪ β} {l : α →ᵀ M} : embDomain f l = ⊤ ↔ l = ⊤ :=
--   (embDomain_injective f).eq_iff' <| embDomain_top f

-- theorem embDomain_mapRange (f : α ↪ β) (g : M → N) (p : α →ᵀ M) (hg : g ⊤ = ⊤) :
...
-- theorem single_of_embDomain_single (l : α →ᵀ M) (f : α ↪ β) (a : β) (b : M) (hb : b ≠ ⊤)
--     (h : l.embDomain f = single a b) : ∃ x, l = single x b ∧ f x = a := by
--   classical
--     have h_map_support : Finset.map f l.support = {a} := by
--       rw [← support_embDomain, h, support_single_ne_top _ hb]
