...
open EffectfulStruct

class Slides [Category C] [MonoidalCategoryStruct C] [O : ∀X Y : C, PartialOrder (X ⟶ Y)]
  (L R : MorphismProperty C) where
  left_sliding_fwd : ∀ {X Y X' Y' : C} (f : X ⟶ Y) (g : X' ⟶ Y'), L f → R g → f ⋉ g ≤ f ⋊ g
  right_sliding_fwd : ∀ {X Y X' Y' : C} (f : X ⟶ Y) (g : X' ⟶ Y'), L f → R g → g ⋊ f ≤ g ⋉ f

class EnrichedEffectful
  (C : Type v) [Category C] [MonoidalCategoryStruct C] [BraidedCategoryStruct C]
  ...
  : Prop
  where
  eff_slides : ∀e₁ e₂ : E, e₁ ‖ e₂ → Slides (C := C) (eff e₁) (eff e₂)
...
class Effectful
  ...
  [EffectfulStruct C E]
  : Prop where
  eff_commutes : ∀e₁ e₂ : E, e₁ ‖ e₂ → Commutes (C := C) (eff e₁) (eff e₂)
