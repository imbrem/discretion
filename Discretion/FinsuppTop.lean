...
/-- `FinsuppTop α M`, denoted `α →ᵀ M`, is the type of functions `f : α → M` such that
  `f x = ⊤` for all but finitely many `x`. -/
structure FinsuppTop (α : Type*) (M : Type*) [Top M] where
  /-- The support of a finitely supported function (aka `FinsuppTop`). -/
  support : Finset α
  /-- The underlying function of a bundled finitely supported function (aka `FinsuppTop`). -/
  toFun : α → M
  /-- The witness that the support of a `FinsuppTop` is indeed the exact locus where its
  underlying function is not `⊤`. -/
  mem_support_toFun : ∀ a, a ∈ support ↔ toFun a ≠ ⊤


@[inherit_doc]
infixr:25 " →ᵀ " => FinsuppTop
...
instance instFunLike : FunLike (α →ᵀ M) α M :=
  ⟨toFun, by
...
directly. -/
instance instCoeFun : CoeFun (α →ᵀ M) fun _ => α → M :=
  inferInstance

@[ext]
theorem ext {f g : α →ᵀ M} (h : ∀ a, f a = g a) : f = g :=
  DFunLike.ext _ _ h

@[simp, norm_cast]
theorem coe_mk (f : α → M) (s : Finset α) (h : ∀ a, a ∈ s ↔ f a ≠ ⊤) : ⇑(⟨s, f, h⟩ : α →ᵀ M) = f :=
  rfl

instance instTop : Top (α →ᵀ M) :=
  ⟨⟨∅, ⊤, fun _ => ⟨fun h ↦ (not_mem_empty _ h).elim, fun H => (H rfl).elim⟩⟩⟩

@[simp, norm_cast] lemma coe_top : ⇑(⊤ : α →ᵀ M) = ⊤ := rfl

theorem top_apply {a : α} : (⊤ : α →ᵀ M) a = ⊤ :=
  rfl
...
instance instInhabited : Inhabited (α →ᵀ M) :=
  ⟨⊤⟩

@[simp]
theorem mem_support_iff {f : α →ᵀ M} : ∀ {a : α}, a ∈ f.support ↔ f a ≠ ⊤ :=
  @(f.mem_support_toFun)

@[simp, norm_cast]
theorem fun_support_eq (f : α →ᵀ M) : Function.infSupport f = f.support :=
  Set.ext fun _x => mem_support_iff.symm

theorem not_mem_support_iff {f : α →ᵀ M} {a} : a ∉ f.support ↔ f a = ⊤ :=
  not_iff_comm.1 mem_support_iff.symm

@[simp, norm_cast]
theorem coe_eq_top {f : α →ᵀ M} : (f : α → M) = ⊤ ↔ f = ⊤ := by rw [← coe_top, DFunLike.coe_fn_eq]

theorem ext_iff' {f g : α →ᵀ M} : f = g ↔ f.support = g.support ∧ ∀ x ∈ f.support, f x = g x :=
...
@[simp]
theorem support_eq_empty {f : α →ᵀ M} : f.support = ∅ ↔ f = ⊤ :=
  by simp [Finset.ext_iff, DFunLike.ext_iff]

theorem support_nonempty_iff {f : α →ᵀ M} : f.support.Nonempty ↔ f ≠ ⊤ := by
  simp only [support_eq_empty, Finset.nonempty_iff_ne_empty, Ne]

theorem card_support_eq_zero {f : α →ᵀ M} : card f.support = 0 ↔ f = ⊤ := by simp

instance instDecidableEq [DecidableEq α] [DecidableEq M] : DecidableEq (α →ᵀ M) := fun f g =>
  decidable_of_iff (f.support = g.support ∧ ∀ a ∈ f.support, f a = g a) ext_iff'.symm

theorem support_subset_iff {s : Set α} {f : α →ᵀ M} :
    ↑f.support ⊆ s ↔ ∀ a ∉ s, f a = ⊤ := by
  simp only [Set.subset_def, mem_coe, mem_support_iff]; exact forall_congr' fun a => not_imp_comm

-- TODO: this can actually be made computable!
/-- Given `Finite α`, `equivFunOnFinite` is the `Equiv` between `α →ᵀ β` and `α → β`.
  (All functions on a finite type are finitely supported.) -/
@[simps]
noncomputable def equivFunOnFinite [Finite α] : (α →ᵀ M) ≃ (α → M) where
  toFun := (⇑)
  invFun f := mk (Function.infSupport f).toFinite.toFinset f fun _a => Set.Finite.mem_toFinset _
  left_inv _f := ext fun _x => rfl
  right_inv _f := rfl

@[simp]
theorem equivFunOnFinite_symm_coe {α} [Finite α] (f : α →ᵀ M) : equivFunOnFinite.symm f = f :=
  equivFunOnFinite.symm_apply_apply f

/--
If `α` has a unique term, the type of finitely supported functions `α →ᵀ β` is equivalent to `β`.
-/
@[simps!]
noncomputable def _root_.Equiv.finsuppTopUnique {ι : Type*} [Unique ι] : (ι →ᵀ M) ≃ M :=
  FinsuppTop.equivFunOnFinite.trans (Equiv.funUnique ι M)

@[ext]
theorem unique_ext [Unique α] {f g : α →ᵀ M} (h : f default = g default) : f = g :=
  ext fun a => by rwa [Unique.eq_default a]

theorem unique_ext_iff [Unique α] {f g : α →ᵀ M} : f = g ↔ f default = g default :=
...
variable [Top M] [DecidableEq α] [DecidableEq β] [DecidableEq M] {a a' : α} {b : M}
